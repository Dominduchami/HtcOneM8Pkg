...
if [ $DEVICE == 'All' ]; then
	...
	do
		if [ $PlatformName != 'All' ]; then
			...
			GCC_ARM_PREFIX=arm-linux-gnueabihf- build -s -n 0 -a ARM -t GCC -p Platforms/Htc${PlatformName}/Htc${PlatformName}Pkg.dsc
			./build_boot_shim.sh
	...
	GCC_ARM_PREFIX=arm-linux-gnueabihf- build -s -n 0 -a ARM -t GCC -p Platforms/Htc${DEVICE}/Htc${DEVICE}Pkg.dsc
